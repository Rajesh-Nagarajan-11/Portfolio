...
export default function ProjectsSection() {
  /* heading + line reveal */
  const headingRef = useGsapReveal<HTMLDivElement>('[data-h]', {
    stagger: 0.15,
    duration: 1,
    y: 30,
    blur: true,
  ...
  /* card grid reveal — each card staggers in */
  const gridRef = useGsapReveal<HTMLDivElement>('[data-card]', {
    stagger: 0.13,
    duration: 0.85,
    y: 50,
    blur: false,
    ...
    <>
      <section className="relative w-full bg-black py-20 md:py-28 overflow-hidden">
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 sm:px-8 md:px-10">
          ...
          {/* Heading */}
          <div ref={headingRef} className="flex flex-col items-center justify-center mb-16 md:mb-20">
            <h2
              data-h
              className={`${cormorant.className} text-white text-4xl sm:text-5xl md:text-6xl font-medium tracking-tight text-center`}
            >
              Projects
            </h2>
            <span data-h className="block h-[2px] w-12 bg-[#39D353] mt-4 shadow-[0_0_10px_#39D353]" />
          </div>
          ...
          <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
